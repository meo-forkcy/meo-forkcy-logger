...
const { clearConsole } = require("meo-forkcy-utils");

const socialRows = [
  ["💬 Telegram (Me)", "https://t.me/MeoMunDep"],
  [" 𝕏 Twitter - X", "https://x.com/Meomundep"],
...
/**
 * Splits a string into chunks of a maximum width.
 *
 * @param {string} val - The input string to wrap.
 * @param {number} [maxWidth=40] - The maximum length of each chunk.
 * @returns {string[]} An array of substrings, each with a length up to maxWidth.
 *
 * @example
 * wrapValue("HelloWorld", 5);
 * // ["Hello", "World"]
 *
 * @example
 * wrapValue("short", 10);
 * // ["short"]
 */
function wrapValue(val, maxWidth = 40) {
  if (val.length <= maxWidth) return [val];
  const chunks = [];
  for (let i = 0; i < val.length; i += maxWidth) {
    chunks.push(val.slice(i, i + maxWidth));
  }
  return chunks;
}
 ...
 */
async function printMergedTableWithBanner(
  botName,
  configs = {},
  colorL1 = colors.lime,
  colorL2 = colors.cyan,
  ...
  const configRows = Object.entries(configs).flatMap(([k, v]) => {
    let val;
    ...
    const wrapped = wrapValue(val, 50); // Limit to 50 characters per line
    return wrapped.map((line, idx) => (idx === 0 ? [k, line] : ["", line]));
  });
